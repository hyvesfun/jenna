...
namespace OAuth
{
    public class OAuthBase
    {
        ...
        public string GenerateSignatureBase(Uri url, Dictionary<string, string> parameters, string consumerKey, string consumerSecret, string token, string tokenSecret, string httpMethod, string timeStamp, string nonce, string signatureType)
        {
            if (token == null)
            {
                token = string.Empty;
        ...
        /// <param name="httpMethod">The http method used. Must be a valid HTTP method verb (POST,GET,PUT, etc)</param>
        /// <returns>A base64 string of the hash value</returns>
        public string GenerateSignature(Uri url, Dictionary<string, string> parameters, string consumerKey, string consumerSecret, string token, string tokenSecret, string httpMethod, string timeStamp, string nonce)
        {
        ...
        /// <returns>A base64 string of the hash value</returns>
        public string GenerateSignature(Uri url, Dictionary<string, string> parameters, string consumerKey, string consumerSecret, string token, string tokenSecret, string httpMethod, string timeStamp, string nonce, SignatureTypes signatureType)
        {
        ...
        public virtual string GenerateNonce()
        {
            ...
            var b = new byte[4];
            g.GetBytes(b);
            int value = b[0] << 24 | b[1] << 16 | b[2] << 8 | b[3];
            value = value%1000;
            // Just a simple implementation of a random number between 123400 and 9999999
            return (random.Next(123400, 9990000) +value).ToString();
        }
    }
}
